...
<head>
    ...
    @livewireStyles
    <style>
        #section-dashboard, #section-students, #section-employees, #section-structure,
        #section-audit-logs, #section-users-management,
        [onclick*="add-student-tab"], [onclick*="add-employee-tab"],
        [onclick*="showSection('dashboard'"], [onclick*="showSection('students'"], [onclick*="showSection('employees'"],
        [onclick*="showSection('structure'"], [onclick*="showSection('audit-logs'"], [onclick*="showSection('users-management'"] {
...
<nav class="sidebar">
    ...
    <div class="sidebar-section">
        ...
        <button class="nav-item" onclick="window.location.href='http://127.0.0.1:8000/#dashboard'">
    ...
    <div class="sidebar-section">
        ...
        <button class="nav-item" onclick="window.location.href='http://127.0.0.1:8000/#students'">
            <span class="icon">🎓</span> {{ $t('Students', 'الطلاب') }}
            <span class="nav-badge">{{ $totalStudents }}</span>
        </button>
        <button class="nav-item" onclick="window.location.href='http://127.0.0.1:8000/#employees'">
            <span class="icon">💼</span> {{ $t('Employees', 'الموظفون') }}
            <span class="nav-badge">{{ $totalEmployees }}</span>
        </button>
        <button class="nav-item" onclick="window.location.href='http://127.0.0.1:8000/#structure'">
    ...
    <div class="sidebar-section">
        ...
        <button class="nav-item active" id="nav-item-research-groups" onclick="showSection('research-groups', this)">
            <span class="icon">🔬</span> {{ $t('Research Groups', 'المجموعات البحثية') }}
            <span class="nav-badge">{{ $totalResearchGroups }}</span>
        </button>
        <button class="nav-item" id="nav-item-projects" onclick="showSection('projects', this)">
            <span class="icon">📁</span> {{ $t('Projects', 'المشاريع البحثية') }}
            <span class="nav-badge">{{ $totalProjects }}</span>
        </button>
        <button class="nav-item" id="nav-item-publications" onclick="showSection('publications', this)">
    ...
    <div class="sidebar-section">
        ...
        <button class="nav-item" id="nav-item-audit-logs" onclick="window.location.href='http://127.0.0.1:8000/#audit-logs'">
            <span class="icon">📋</span> {{ $t('Audit Logs', 'سجلات التدقيق') }}
        </button>
        @endif
        @if(auth()->user()->hasAnyRole(['SYSTEM_ADMIN', 'UNIVERSITY_ADMIN']))
        <button class="nav-item" id="nav-item-users-management" onclick="window.location.href='http://127.0.0.1:8000/#users-management'">
...
<script>
    ...
    window.onload = function() {
        ...
        if (hash) {
            const navBtn = document.getElementById('nav-item-' + hash) || document.querySelector(`[onclick*="${hash}"]`);
            if (navBtn) {
                showSection(hash, navBtn);
                return;
            }
        }
        ...
        @if(session('active_section'))
            ...
            const navBtn = document.getElementById('nav-item-' + activeSec) || document.querySelector(`[onclick*="${activeSec}"]`);
        ...
        @else
            const navBtn = document.getElementById('nav-item-research-groups');
            showSection('research-groups', navBtn);
        @endif
    };
</script>
@livewireScripts
